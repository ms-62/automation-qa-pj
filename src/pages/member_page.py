...
class MemberPage(BasePage):        
    ...
    def open_email_edit_form(self, timeout=5) -> bool:
        ...
        logger.error("'이메일' 수정 버튼 찾음, 클릭 시도")
        ...
        if not input_email:
            logger.error("이메일 수정 폼 열기 실패 (입력 필드 미노출)")
            return False
    ...
    def member_email(self, email) -> bool:
        input_email = self.scroll_and_interact((By.NAME,
            NAME["INPUT_EMAIL"]),offset=100,timeout=3,condition="clickable")
        if not input_email:
    ...
    def certification_email(self) -> bool:
        ...
        email_row = self.scroll_and_interact((By.XPATH,XPATH["EMAIL_ROW"]),condition="clickable", offset=120, timeout=3)
        
        if not email_row:
            self.driver.execute_script("window.scrollTo({top: 0, behavior: 'instant'});")
        logger.info("인증 버튼 JS 클릭 + 이메일 행으로 복귀")
        try:
            input_email = self.wait_for_element(
                By.NAME,
                NAME["INPUT_EMAIL"],
